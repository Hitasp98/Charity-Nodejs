(function($) {
  ...
  // Start Gallery Color
  if($('.color-glllery').length){

    function imgprocess(){
      var img= $('.color-glllery img');
      img.each(function(index){
      var colorThief = new ColorThief();
      var mcolor = colorThief.getColor(img[index]);
      var pallet = colorThief.getPalette(img[index], 6);
      var text = $(this).siblings('.overlay-text');
      var maincolor = $(this).siblings('.colors').children('.main-color');
      var palletlist = $(this).siblings('.colors').children('.pallet').children('ul');
      
      text.css('background','rgba('+mcolor[0]+','+mcolor[1]+','+mcolor[2]+',.7)');
      maincolor.css('background','rgb('+mcolor[0]+','+mcolor[1]+','+mcolor[2]+')');
        $.each(pallet,function(index){
          $('<li />').appendTo(palletlist).css('background','rgb('+$(this)[0]+','+$(this)[1]+','+$(this)[2]+')');
        })
      })
    }

    $('body').imagesLoaded(imgprocess);
  }
  ...
  $('.sidecontact').click(function(){
    $('#sidecontentinfo').addClass('active');
  })
  $('#sidecontentinfo span.close').click(function(){
    $('#sidecontentinfo').removeClass('active');
  })
  // End Sidecontact
...
// Start sid menu
$('.sidebarmenu > ul li span.drop').click(function(){
  if($(this).hasClass('active')){
    $(this).parent().find('ul').slideUp();
    $(this).parent().find('span.drop').removeClass('active');
  }else{
...
// Custom  scrolbar
if($('#sidebar2 > div').length){

$("#sidebar2 > div").mCustomScrollbar({
          axis:"y",
          theme:"light-thick",
          scrollInertia: 200,
...
$grid.imagesLoaded().progress( function() {
  $grid.masonry('layout');
     ...
     // Start Blog Massonry
if($('#massonry-blog').length){

  var $grid = $('#massonry-blog').masonry({
...
$('a.loadmore').click(function(e){
  ...
  $.get(url,function(data){
    var boxes= $(data).find('.massonry-item');
...
$('#random-news .close').click(function(){
  $('#random-news').remove();
})
randomnews();
