      ...
      requestApi.post(
        {
          ...
          form: { PersonId: findRequest[0].PersonId },
          url: "http://localhost:8090/tblCommonBaseData/tblPayment",
          form: { PersonId: findRequest[0].PersonId },
          url: "http://localhost:8090/tblCommonBaseData/tblDistributionGoods",
          form: { PersonId: findRequest[0].PersonId },
          url: "http://localhost:8090/tblCommonBaseData/tblNonCashRequest",
          form: { PersonId: findRequest[0].PersonId },
          url: "http://localhost:8090/tblCommonBaseData/tblNeedyAccounts",
          form: { PersonId: findRequest[0].PersonId },
        },
        async function(err, res, body) {
          if (
            (await JSON.parse(body).CommonBaseTypeId) == findRequest[0].PersonId
          ) {
            response.json({
            ...
            let checked = await PersonalModels.ws_loadPersonal(findRequest);
            if (checked[0] != " ") {
              await tblCommonBaseTypeModel
                .deletePersonalController(findRequest)
                .then(result => {
                  if (result == 1) {
