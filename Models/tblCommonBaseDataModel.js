...
async function getTblCommonBaseData(findRequest) {
  try {
    ...
    if (
      findRequest.CommonBaseDataId == null &&
      findRequest.BaseCode == null &&
      findRequest.BaseValue == null &&
      findRequest.BaseValue == null &&
      findRequest.CommonBaseTypeId == null
    ) {
      getTblCommonBaseData = await pool
        .request()
        .query(`select * from tblCommonBaseData`);
      ...
      getTblCommonBaseData = await pool
        .request()
        .query(`select * from tblCommonBaseData where` + whereclause);
...
async function insertTblCommonBaseData(findRequest) {
  try {
    ...
    let pool = await sql.connect(config);
    let getTblCommonBaseTypeCode = await pool
      .request()
      .query(
        `select * from tblCommonBaseType where CommonBaseTypeId =` +
          findRequest["CommonBaseTypeId"]
      );

    let getTblCommonBaseCode =
      getTblCommonBaseTypeCode.recordsets[0][0].BaseTypeCode +
      getRandomString.fnGetRandomString(3);

    //insert data
    if (findRequest.BaseValue != null) {
      let insertTblCommonBaseData = await pool.request().query(
        `INSERT INTO tblCommonBaseData (BaseCode,BaseValue,CommonBaseTypeId)
                VALUES (` +
          "'" +
          getTblCommonBaseCode +
          "'" +
          `,N` +
          "'" +
          findRequest.BaseValue +
          "'" +
          `,` +
          findRequest["CommonBaseTypeId"] +
          `)`
      );
      let getTblCommonBaseData = await pool
        .request()
        .query(
          `select * from tblCommonBaseData where BaseValue = N` +
            "'" +
            findRequest.BaseValue +
            "'" +
            " and " +
...
async function updateTblCommonBaseData(findRequest) {
  try {
    ...
    let getTblCommonBaseDataCode = await pool
      .request()
      .query(
        `select * from tblCommonBaseData where CommonBaseDataId =` +
          findRequest["CommonBaseDataId"]
      );
    let getTblCommonBaseCode =
      getTblCommonBaseTypeCode.recordsets[0][0].BaseTypeCode +
      (await getTblCommonBaseDataCode.recordsets[0][0].BaseCode.substring(
        3,
        6
      ));
    ...
    for (x in findRequest) {
      if (x == "CommonBaseDataId") {
        ...
        typeof findRequest[String(x)] == "string"
      ) {
        updateTblCommonBaseData = await pool.request().query(
          `UPDATE tblCommonBaseData
                        SET  ${x} = N` +
            "'" +
            findRequest[String(x)] +
            "'" +
    ...
    updateTblCommonBaseData = await pool
      .request()
      .query(
        `select * from tblCommonBaseData where CommonBaseDataId =` +
          CommonBaseDataValue
...
async function deleteTblCommonBaseData(findRequest) {
  try {
    ...
    let getTblCommonBaseData = await pool
      .request()
      .query(
        `select * from tblCommonBaseData where CommonBaseDataId =  ${findRequest.CommonBaseDataId};`
      );

    if (getTblCommonBaseData != null) {
      deleteTblCommonBaseData = await pool
        .request()
        .query(
          `DELETE FROM tblCommonBaseData WHERE CommonBaseDataId = ${findRequest.CommonBaseDataId};`
        );
    }
